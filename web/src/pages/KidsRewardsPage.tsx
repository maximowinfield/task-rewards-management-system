...
export default function KidsRewardsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ✅ If no kid selected yet, default to first kid
        if (!selectedKidId && list.length > 0) {
          const firstId = String(list[0].id);
          setAuth((prev: any) => ({
            ...prev,
            selectedKidId: firstId,
            selectedKidName: list[0].displayName,
          }));
        }
      } catch (e) {
  ...
  }, [auth?.parentToken, isParentMode]);

  // ✅ Sync URL kidId -> auth selectedKidId (Parent only)
  useEffect(() => {
    if (!kidId) return;
    if (auth?.activeRole !== "Parent") return;

    if (auth?.selectedKidId !== kidId) {
      const kid = kids.find((k) => String(k.id) === String(kidId));

      setAuth((prev: any) => ({
        ...prev,
        selectedKidId: kidId,
        selectedKidName: kid?.displayName ?? prev.selectedKidName,
      }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kidId, kids, auth?.activeRole]);
  ...
  if (!effectiveKidId) {
    ...
    return (
      ...
      >
        ...
        <p style={{ marginTop: 0, color: ui.subtleText }}>Pick a kid to continue.</p>
        ...
        <div
          style={{
            ...
            alignItems: "center",
            marginTop: 16,
            boxShadow: isDark ? "0 0 0 rgba(0,0,0,0)" : "0 1px 3px rgba(15, 23, 42, 0.08)",
          }}
        >
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", fontFamily: "system-ui" }}>
        ...
        {error && (
          ...
          >
            ...
            <pre style={{ margin: "8px 0 0", whiteSpace: "pre-wrap", color: ui.dangerText }}>
              {error}
            </pre>
          </div>
        )}
        ...
        >
          <div style={{ display: "grid", gap: 4 }}>
            ...
            <div style={{ color: ui.subtleText, fontSize: 13 }}>
              Viewing Kid ID: <strong style={{ color: ui.text }}>{effectiveKidId}</strong> • Mode:{" "}
              <strong style={{ color: ui.text }}>{auth?.uiMode ?? "Kid"}</strong>
            </div>
          </div>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 12, alignItems: "center" }}>
            ...
            {isParentMode && (
              <button
                onClick={() => navigate("/parent/select-kid")}
                style={{
                  border: `1px solid ${ui.border}`,
                  background: ui.buttonBg,
                  color: ui.buttonText,
